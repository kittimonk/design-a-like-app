...
CANON_MAP = {
    ...
    r"(?i)^target db name/outgoing file path.*$": "tgt_db",
        r"(?i)^target schema name.*auto.*$": "tgt_schema",
        r"(?i)^target table/file name.*auto.*$": "tgt_table",
        r"(?i)^target column/field name.*auto.*$": "tgt_column",
        r"(?i)^target data type.*auto.*$": "tgt_datatype",
...
def build_summary(csv_path: str, nlp_json_path: str, outdir: str):
    ...
    summary = {}
    # include any table that appears as a source or as an explicit target
    all_tables = sorted(set(list(df["src_table"]) + list(df["tgt_table"])))
    for src in all_tables:
        if not src:
            continue
        alias = alias_map.get(src, (src[:4] if src else "src")).lower()
        # include rows where this table is either the source table or the target table
        sdf = df[(df["src_table"] == src) | (df["tgt_table"] == src)]
        ...
        # Referenced + lineage
        # known columns: learned from source side plus any explicit target columns in the mapping
        known_from_src = set([c.lower() for c in known_cols_by_src.get(src, set())])
        known_from_tgt = set([s(r.get("tgt_column", "")).lower() for _, r in df[df["tgt_table"] == src].iterrows() if s(r.get("tgt_column", ""))])
        known_cols = known_from_src.union(known_from_tgt)
