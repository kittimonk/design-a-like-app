...
Current end-to-end steps:

  1) nlp_rules_parser_v6.py
       → nlp_rules_interpretation_v6.json

  2) (MANUAL / PRE-RUN) extract_sources_columns_v7c.py
       → source_columns_summary_v7c.json
     - This is NOT called inside this script because you sometimes
       hand-edit v7c (aliases, join fixes, etc.).
     - This runner expects an existing source_columns_summary_v7c.json
       in the output directory.

  3) lookup_reference_module_v3.py
       → lookup_reference_module_v3.txt  (lookup view blocks)

  4) data_sourcing_cte_v11a.py
       → data_sourcing_module_v11a.txt

  5) build_model_v1_v11c.py
       → model_v1.json

  6) build_dt_view_from_model_v12_multi_lkp.py
       → dt_<malcode>_<base>.json
       (lookup-aware, multi-lookup per Option C)

  7) build_job_json_v1.py
       → dt_<malcode>_<base>.job.txt

  8) build_load_enrich_from_dt_v2.py
       → load_enrich_<malcode>_<base>.job.txt

  9) build_combined_job_v5.py
...
def main():
    ...
    # extract_v7c     = scripts_dir / "extract_sources_columns_v7c.py"  # manual / external
    ...
    dt_view_v12       = scripts_dir / "build_dt_view_from_model_v12_multi_lkp.py"
    build_job_v1      = scripts_dir / "build_job_json_v1.py"
    load_enrich_v2    = scripts_dir / "build_load_enrich_from_dt_v2.py"
    combined_v5       = scripts_dir / "build_combined_job_v5.py"
    ...
    # 2) v7c is assumed to be pre-generated & possibly hand-edited.
    #    Make sure it exists; if not, instruct the user.
    if not v7c_json.exists():
        raise FileNotFoundError(
            f"Expected {v7c_json} to exist.\n"
            "Please run extract_sources_columns_v7c.py manually first, e.g.:\n"
            f"  python extract_sources_columns_v7c.py \"{csv_path}\" --nlp_json \"{nlp_json}\" --outdir \"{outdir}\"\n"
            "Then re-run run_full_job_v1.py."
        )

    # 3) Lookup reference module v3 (lookup views)
    run(
        [
            sys.executable,
            str(lookup_ref_v3),
    ...
    # 6) Build dt_<malcode>_<base>.json from v12 multi-lookup script
    ...
    print("     cw_<job-id>_<target_table>_<malcode>.txt (printed by build_combined_job_v5.py)")
