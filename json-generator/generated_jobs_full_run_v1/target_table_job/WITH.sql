...
SELECT
    '+00331 (Asset).' AS asset_liblty_CD,
    '-2 (No Source).' AS compnd_int_CD,
    '+00616 (CAD) WHEN mas' AS curncy_CD,
    CASE
  ...
  END AS ERLCSH_ELIGBL_IN
    ...
    'NULL it the date is invalid or '0001-01-01' END AS FIRST_OFFER_DT FROM ossbr_2_1 mas' AS first_offer_dt,
  ...
  END AS tantrum_family_cd LEFT JOIN GLSXREF ref ON mas.SRSECCODE = ref.sm_SECURITY_CODE AS tantrum_family_CD,
    ...
    CASE
   -- Check if ossbr_2_1.SRSECCODE matches GLSXREF.sm_SECURITY_CODE 
 
    WHEN ref.sm_SECURITY_CODE IS NOT NULL AND LEFT(ref.SEND_CD, 3) IN ('SBM', 'SBG', 'SBS','SBB', 'SBC') 
      THEN 'Y' -- Match found and SEND_CD starts with specified values 
    WHEN LEFT(mas.SRSHSBESE, 2) = 'SB' 
      THEN 'Y' -- First two characters of SRSHSBESE are 'SB' 
    ELSE 'N' -- Default to 'N' 
